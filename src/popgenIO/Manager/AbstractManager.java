...
abstract class AbstractManager implements Manager<Boolean>, Cloneable, Serializable {
	...
	public AbstractManager(DataSet<Boolean> dataset) {
		allData = dataset;
		clearPredictions();
	}

	@Override
	protected AbstractManager clone() {
		try {
			AbstractManager other = (AbstractManager) super.clone();
	...
	@Override
	public Boolean[] getPrediction(Site ss, Diplotype dd) {
	...
	@Override
	public Boolean[] getPrediction(Site ss, Genotype gg) {
	...
	@Override
	public DataSet<Boolean> getAllData() {
		return allData.clone();
	}

	@Override
	public DataSet<Boolean>getTrainingSet() {
		DataSet<Boolean>trainset = allData.clone();
	...
	@Override
	public DataSet<Boolean>getTestSet() {
		boolean hastest = false;
		DataSet<Boolean>testset = allData.clone();
		assert testset != null;
	...
	public double getMeanAllele(Site ss) {
		...
		for (Genotype gg : allData.getGenotypes()) {
			if (allData.isObserved(ss, gg)) {
				numerator += allData.get(ss, gg)[0]?1.0:0.0;
				numerator += allData.get(ss, gg)[1]?1.0:0.0;
				denominator += 2.0;
			}
		}
		...
		for (Diplotype dd : allData.getDiplotypes()) {
			if (allData.isObserved(ss, dd)) {
				numerator += allData.get(ss, dd)[0]?1.0:0.0;
				numerator += allData.get(ss, dd)[1]?1.0:0.0;
				denominator += 2.0;
			}
		}
		...
		for (Haplotype hh : allData.getHaplotypes()) {
			if (allData.isObserved(ss, hh)) {
				numerator += allData.get(ss, hh)?1.0:0.0;
				denominator += 1.0;
			}
		}
